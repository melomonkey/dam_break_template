...
{
	fstream fin; 
	fin.open("INPUT_MODELS\\FILE0.txt", ios::in);
	...
	for(int i = 0; i < coordination.size(); ++ i) 
	{
		...
		tmp.mass.val[0] = 0.00015; tmp.mass.val[1] = tmp.mass.val[0]; tmp.mass.val[2] = tmp.mass.val[0];
		...
		tmp.smthR.val[0] = 0.01 * 1.5; tmp.smthR.val[1] = tmp.smthR.val[0]; tmp.smthR.val[2] = tmp.smthR.val[0];
